...
function setTableVal(data) {
    $.each(data.limitItems, function(i, period) {
        $.each(period, function(j, items) {
            ...
            e_li.append("<img src='" + getPicUrl("item", items[0]) + "' style='height:30px;width:auto;'/>");
            e_li.append(" × " + items[1]);
    ...
    $.each(data.limitQPs, function(i, qp) {
        var e_li = $("<li></li>");
        e_li.attr("class", "require_item");
        e_li.append("<img src='http://www.z4a.net/images/2017/11/01/QP.png' style='height:30px;width:auto;'/>");
